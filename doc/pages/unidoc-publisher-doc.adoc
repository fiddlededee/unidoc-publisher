...
As an example you may download link:{src-root}/doc/build-doc.kts[the Kotlin script file]. It is used to build the current documentation (prerequisites: Java 11+ and Kotlin 19+). The script runs with the following CLI parameters.
...
<5> Parsing HTML into AST
<6> Correcting image relative paths
...
image::../images/4to6.png[width=40%]
...
+
[[mvp-doc]]
Creates this documentation from the Asciidoc file link:{src-root}/doc/pages/unidoc-publisher.adoc[/doc/pages/unidoc-publisher.adoc].
. link:{src-root}/example/ps-118/ps-118.main.kts[example/ps-118/ps-118.main.kts]
+
[[ps-118]]
Parses HTML-files and unites them into one book of A4 format (link:ps-118.fodt[FODT], link:ps-118.pdf[PDF], link:ps-118.docx[DOCX]), and the electronic book format (link:ps-118-ebook.fodt[FODT], link:ps-118-ebook.pdf[PDF]), adjusting the page layout for each format.
+
...
+
[[from-scratch]]
Builds the letter from JSON data and converts it to link:letter.fodt[FODT], link:letter.odt[ODT], link:letter.pdf[PDF] and link:letter.pdf[DOCX] formats.
Adapted from https://pandoc.org/using-the-pandoc-api.html#builder[Pandoc documentation].
. link:{src-root}/example/wr/writerside.main.kts[example/builder/writerside.main.kts]
+
[[writer-side]]
Converts "`Export to PDF`" page from Writerside tutorial into link:export-to-pdf.fodt[FODT], link:export-to-pdf.odt[ODT], link:export-to-pdf.pdf[PDF] and link:export-to-pdf.docx[DOCX] formats.
...
To run examples you need the Kotlin compiler. To convert FODT file to PDF, ODT, DOCX format https://github.com/CourseOrchestra/lo-kts-converter[a special LibreOffice wrapper in Kotlin script] is used. But you may do the conversion manually or use any LibreOffice wrapper converter of your choice.
...
* https://github.com/redundent/kotlin-xml-builder[Kotlin XML Builder], Copyright 2015 the original author or authors, licensed under Apache License, Version 2.0
* https://github.com/FasterXML/jackson[Jackson], licensed under Apache License, Version 2.0
